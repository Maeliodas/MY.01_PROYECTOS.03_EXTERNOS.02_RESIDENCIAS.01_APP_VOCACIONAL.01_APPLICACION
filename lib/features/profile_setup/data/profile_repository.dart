  ...
  Future<void> saveLanguages({
    ...
    await db.transaction((txn) async {
      ...
      for (final id in motherIds) {
        await txn.insert(Tables.profileLanguages, {'language_id': id, 'type': 'mother'});
      }
      for (final name in customMother) {
        await txn.insert(Tables.profileLanguages, {'type': 'mother', 'custom_name': name});
      }
      for (final id in foreignIds) {
        await txn.insert(Tables.profileLanguages, {'language_id': id, 'type': 'foreign'});
      }
      for (final name in customForeign) {
        await txn.insert(Tables.profileLanguages, {'type': 'foreign', 'custom_name': name});
      }
    });
  }
}
