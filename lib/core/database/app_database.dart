...
class AppDatabase {
  ...
  Future<void> _create(Database db, int version) async {
    ...
    await db.execute('CREATE TABLE ${Tables.profile}(id TEXT PRIMARY KEY, name TEXT NOT NULL, age INTEGER NOT NULL, gender TEXT NOT NULL, state TEXT NOT NULL DEFAULT \'No especificado\', school TEXT NOT NULL DEFAULT \'No especificada\', speaks_languages INTEGER NOT NULL DEFAULT 0, languages_list TEXT NOT NULL DEFAULT \'\', avatar_config_json TEXT NOT NULL DEFAULT \'{}\', created_at TEXT NOT NULL, updated_at TEXT NOT NULL)');
  ...
  Future<void> _upgrade(Database db, int oldVersion, int newVersion) async {
    ...
    if (oldVersion < 3) {
      // La versión anterior usaba school_id y un perfil parcial. Estas columnas
      // permiten que UserProfile y el repositorio compartan el mismo esquema.
      await db.execute('ALTER TABLE ${Tables.profile} ADD COLUMN state TEXT NOT NULL DEFAULT \'No especificado\'');
      await db.execute('ALTER TABLE ${Tables.profile} ADD COLUMN school TEXT NOT NULL DEFAULT \'No especificada\'');
      await db.execute('ALTER TABLE ${Tables.profile} ADD COLUMN speaks_languages INTEGER NOT NULL DEFAULT 0');
      await db.execute('ALTER TABLE ${Tables.profile} ADD COLUMN languages_list TEXT NOT NULL DEFAULT \'\'');
      await db.execute('ALTER TABLE ${Tables.profile} ADD COLUMN avatar_config_json TEXT NOT NULL DEFAULT \'{}\'');
      await db.execute('ALTER TABLE ${Tables.profile} ADD COLUMN created_at TEXT NOT NULL DEFAULT \'1970-01-01T00:00:00.000\'');
      await db.execute('ALTER TABLE ${Tables.profile} ADD COLUMN updated_at TEXT NOT NULL DEFAULT \'1970-01-01T00:00:00.000\'');
      // await db.execute('ALTER TABLE ${Tables.profile} ADD COLUMN municipality TEXT NOT NULL DEFAULT \'No especificado\'');
    }

    if (oldVersion < 4) {
      await db.execute('DROP TABLE IF EXISTS ${Tables.results}');
      await db.execute("CREATE TABLE ${Tables.results}(id TEXT PRIMARY KEY, session_id TEXT NOT NULL, score_r REAL NOT NULL DEFAULT 0, score_i REAL NOT NULL DEFAULT 0, score_a REAL NOT NULL DEFAULT 0, score_s REAL NOT NULL DEFAULT 0, score_e REAL NOT NULL DEFAULT 0, score_c REAL NOT NULL DEFAULT 0, holland_code TEXT NOT NULL, top_career_id TEXT NOT NULL DEFAULT '', top_career_name TEXT NOT NULL DEFAULT '', top_career_affinity REAL NOT NULL DEFAULT 0, full_ranking_json TEXT NOT NULL DEFAULT '[]', is_synced INTEGER NOT NULL DEFAULT 0, created_at TEXT NOT NULL)");
    }
  }
  ...
  Future<void> _seed(Database db) async {
    ...
    for (final career in CareersData.initialCareers) {
      await db.insert(Tables.careers, {
        'id': career['id']?.toString() ?? '',
        'name': career['name']?.toString() ?? '',
        'description': career['description']?.toString(),
        'holland_codes': career['holland_codes']?.toString(),
      });
    }
  }
}
